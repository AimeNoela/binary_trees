 ...
 * binary tree by checking whether the node has a parent and whether the node is
 ...
 *
 * Return: Pointer to the sibling node if it exists, or NULL if there is no sibling.
 */
binary_tree_t *binary_tree_sibling(binary_tree_t *node)
{
	...
	if (node->parent->left == node)
		return (node->parent->right); /* If the node is the left child, return the right child (sibling)*/
	return (node->parent->left); /* If the node is the right child, return the left child (sibling)*/
}
